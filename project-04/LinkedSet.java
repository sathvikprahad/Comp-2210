...
public class LinkedSet<T extends Comparable<? super T>> implements Set<T> {
    ...
    * nor null values are allowed. This method ensures that thxe elements in the
   ...
   public boolean add(T element) {
      ...
      if (current.element.compareTo(n.element) > 0) {
         n.next = front;
         n.prev = null;
         n.next.prev = n;
         front = n;
   ...
   private class powerSetIterator implements Iterator<Set<T>> {
      ...
      public boolean hasNext() {
         if (iterationCounter < Math.pow(2, size)) {
            return true;
         }
         return false;
      }
      ...
      public Set<T> next() {
         ...
         LinkedSet<T> toReturn = new LinkedSet<T>();
      
         if (iterationCounter == 0) {
            iterationCounter++;
            return toReturn;
         }
         String binary = Integer.toBinaryString(iterationCounter);
         int length = binary.length() - 1;
         while (length >= 0) {
            if (binary.charAt(length) == '1') toReturn.add(n.element);
            n = n.next;
            length--;
         }
         n = front;
